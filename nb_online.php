...
if ($data[0]) {
    // si on a trouvé un résultat, on modifie le temps du tuple du client en conséquence : en effet, le client vient juste de charger une page WEB, 
	// on modifie alors le temps de son tuple par la date actuelle (en fait le nombre de secondes separant le 1er janvier 1970 de la date actuelle).
	...
	// Dans ce cas, on insère alors dans la table SQL un nouveau tuple comprenant l'adresse IP de ce client ainsi que la date actuelle 
	// (le nombre de secondes entre le 1er janvier 1970 et la date actuelle).
...
// on calcule la date limite pour que l'on considère que les clients soient encore connectés). 
$heure_max = $temps_actuel - $tps_max_connex;

// on prépare une requête SQL permettant de supprimer les clients que l'on considère comme n'étant plus connectés 
// (c'est à dire ayant expiré leur temps de 3 minutes défini comme étant le temps moyen de lecture d'une page WEB).
