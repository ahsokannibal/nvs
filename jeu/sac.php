...
if($dispo){
	...
	if (@$_SESSION["id_perso"]) {
		...
		// recupération des données sur le perso
						...
						if($ok) { // possede plus de 0 objets
							...
							if($testpa >= 1){
								...
								if($bonusPv || $bonusPerception || $bonusPm || $bonusRecup) { 
									...
									// l'objet lui fait recuperer un nombre de pv tel que ses pv après soient inferieurs à ses pvmax 
								...
								// Fiole du berserker
								if($id_o == 9){
									// on enlève les malus de combat au perso
	...
	<?php
			...
			$res = exec_requete($sql);
			$t_nb_objets = mysql_fetch_row($res);
	...
	<table border=0 width=100%>
		...
		<table border=1 width=100%>
			<tr>
				...
				<td width=75%>
					...
					?>
					<img src="../images/pierre_t.png" align="middle">Vous possédez <b><?php echo $or_p; ?></b> pièce<?php if($or_p > 1){echo "s";}?> d'or.<br>
					</center>
				</td>
			</tr>
		</table>
		...
		<table border=1 width=100%>
			...
			$res = exec_requete($sql);
			$nb_obj = mysql_num_rows($res);
			while ($t_obj = mysql_fetch_assoc($res)){
					...
					$res1 = exec_requete($sql1);
					$t_o = mysql_fetch_assoc($res1);
					$nom_o = $t_o["nom_objet"];
					...
					$res2 = exec_requete($sql2);
					$nb_o = mysql_num_rows($res2);
