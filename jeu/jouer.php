						...
						if(in_bat($mysqli, $id_perso)){
							...
							if($pm_perso + $malus_pm >= 1){
								...
								*/
								...
								$sql = "INSERT INTO `evenement` VALUES ('','$id_perso','<font color=$couleur_clan_p>$nom_perso</font>','est sorti du batiment','','','en $xs/$ys',NOW(),'0')";
								$mysqli->query($sql);
											...
											if($nbp[0] != 0){ // si la tour est occupee
												...
												if($pm_perso + $malus_pm >= 1){
													...
													if(!nation_perso_bat($id_perso,$id_inst_bat)) { // pas même nation
														...
														$sql = "INSERT INTO `evenement` VALUES ('','$id_perso','<font color=$couleur_clan_p>$nom_perso</font>','a capturé le batiment $nom_bat','$id_inst_bat','','en $x_bat/$y_bat : Felicitation!',NOW(),'0')";
														$mysqli->query($sql);
													...
													*/
													...
													// mise a jour table evenement
													$sql = "INSERT INTO `evenement` VALUES ('','$id_perso','<font color=$couleur_clan_p>$nom_perso</font>','est entre dans le batiment $nom_bat $id_inst_bat','','','en $x_bat/$y_bat',NOW(),'0')";
													$mysqli->query($sql);
								...
								if (!is_eau_p($fond)) {
									if (!$case_occupee){
										if($pm_perso  + $malus_pm >= $cout_pm){	
											...
											// maj evenement
											$sql = "INSERT INTO `evenement` VALUES ('',$id_perso,'<font color=$couleur_clan_p>$nom_perso</font>','s\'est deplacé','','','en $x_persoN/$y_persoN',NOW(),'0')";
											$mysqli->query($sql);
