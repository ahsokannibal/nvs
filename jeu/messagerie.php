...
$res_a_lire = exec_requete($sql_a_lire);
$a_lire = mysql_num_rows($res_a_lire);
?>
...
if (mysql_num_rows($resultat) == 0) {
	...
	while($row = mysql_fetch_assoc($resultat)) {
		...
		if ($row["lu_message"]){
			echo "<td width=120>" . $row["expediteur_message"] . "</td><td width=150>" . stripslashes($row["date_message"]) . "</td><td colspan=2><a href=lire.php?id=" . $row["id_message"] . "&methode=r>" . stripslashes($row["objet_message"]) . "</a></td>";
		}
		else {
			echo '<td><div class="info">' . $row["expediteur_message"] . '</div></td><td><div class="info">' . stripslashes($row["date_message"]) . '</div></td><td colspan=2><a href=lire.php?id=' . $row["id_message"] . "&methode=r>" . stripslashes($row["objet_message"]) . "</a><b> (non lu)</b></td>";
		}
		echo '</tr>';
		$i++;
	}
...
<tr><td>Que voulez-vous faire des messages sélectionnés?&nbsp;</td><td><input type="submit" name="submit" value="Effacer">&nbsp;</td><td><input type="submit" name="submit" value="Archiver"></td></tr>
</table>
</form>
