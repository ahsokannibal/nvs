		...
		if(filtre_contact($nom_contact) && filtre_contact($noms_contact)){
			...
			if(!$nb_v){
				if(strlen($nom_contact)){
					...
					$sql = "INSERT INTO contact VALUES ('', '$nom_contact', '$noms_contact')";
					$mysqli->query($sql);
				...
				echo "<center><b><font color='red'>Vous possédez déjà un contact de ce nom, veuillez en choisir un autre</font></b></center>";
			}
		}
	}
	else {
...
<img src='../images/vcard_add.png' alt='add contact'> Créer un nouveau contact : <input type='text' name='nom_contact'>
	...
	if(!$nb_v){
		...
		if(strlen($nom_contact)){
			...
			if(filtre_contact($nom_contact)){
				...
				echo "<br /><center><b>Création du contact $nom_contact</b></center><br />";
...
if(isset($_GET['id_contact']) && $_GET['id_contact'] != ""){
	...
	if($verif){
		...
		if($verif_id){
			if(isset($_GET['action']) && $_GET['action'] == "del"){
				...
				echo "<center><b>Contact bien supprimé</b></center>";
			}
			else {
				...
				echo "&nbsp;&nbsp;&nbsp;&nbsp;<b>Contacts $nom_contact</b> :<br />";
				echo "";
				for($i = 0; $i < $taille_contacts; $i++){
	...
	echo "<center><h2>Liste des contacts</h2></center>";
