...
function nouveau_tour_joueur($mysqli, $id_joueur, $new_dla, $clan, $couleur_clan_p) {
	...
	while ($t_persos = $res->fetch_assoc()) {
		...
		if ($pv_perso_nouveau_tour <= 0) {
			...
			$sql = "UPDATE perso SET x_perso='$x', y_perso='$y', pm_perso=$pm_nouveau, pa_perso=paMax_perso/2 + bonusPA_perso, pv_perso=pvMax_perso, or_perso=or_perso+$gain_or, bonusPerception_perso=$bonus_visu, bonus_perso=0, bourre_perso=0, convalescence=0, est_gele=0, gain_xp_tour=0, DLA_perso=FROM_UNIXTIME($new_dla) 
