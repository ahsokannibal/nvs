	...
	public static function validate(array $expected_inputs)
    {
		...
		foreach($expected_inputs as $key => $conditions){
			...
			foreach($conditions as $condition){
				...
				// la condition "bail" permet de stopper à la première erreur rencontrée après le "bail"
				if($condition == 'bail'){
				$bail = 0;
				$current = $key;
				continue;
					...
					switch($condition){
						case 'required':
							if($_POST[$key]==='' || $_POST[$key]===null){
								$errors[$key][$condition] = 'le champ "'.$key.'" est obligatoire';
							}else{
								$bail = 0;
							}
							break;
						case 'numeric':
							if(!is_numeric($_POST[$key])){
								$errors[$key][$condition] = 'le champ "'.$key.'" doit être numérique';
							}else{
								$bail = 0;
							}
							break;
						case 'greater':
							...
							if($item<=$value){
								$errors[$key][$condition] = 'le champ "'.$key.'" doit être supérieur à '.$value.'';
							}else{
								$bail = 0;
							}
							break;
						case 'less':
							...
							if($item>=$value){
								$errors[$key][$condition] = 'le champ "'.$key.'" doit être inférieur à '.$value.'';
							}else{
								$bail = 0;
							}
							break;
						case 'min':
							...
							if($item<$value){
								$errors[$key][$condition] = 'le champ "'.$key.'" doit être supérieur ou égal à '.$value.'';
							}else{
								$bail = 0;
							}
							break;
						case 'max':
							...
							if($item>$value){
								$errors[$key][$condition] = 'le champ "'.$key.'" doit être inférieur ou égal à '.$value.'';
							}else{
								$bail = 0;
							}
							break;
						...
						case 'accepted':
							if(!in_array($_POST[$key],['yes','on',1,true])){
								$errors[$key][$condition] = 'le champ "'.$key.'" doit être accepté';
							}else{
								$bail = 0;
							}
							break;
						case 'same':
							if($_POST[$key]!==$value){
								$errors[$key][$condition] = 'le champ "'.$key.'" est incorrect';
							}else{
								$bail = 0;
							}
							break;
