	...
	<div class="row row-cols-1 row-cols-md-2 mb-4">
		<div class="col">
			<div class="card shadow">
				<div class='card-header'>
					<h3>Afficher une zone ou dessiner sur une zone</h3>
				</div>
				<div class="card-body">
					...
					<?php endif ?>
					<form class='row row-cols-auto align-items-center mb-4' name='showArea' method='post' action='?page=carte&action=edit&id=<?= $id?>'>
						<div class='col-3'>
							<div class='row'>
					...
					<h5 class="card-title mb-3">choix du terrain, pnj ou bâtiment</h5>
					<form class='row row-cols-auto align-items-center' name='drawArea' method='post' action='?page=carte&action=edit&id=<?= $id?>'>
						<input type="hidden" name="_method" value="PUT/PATCH">
						<div class='col-2'>
							<label class="form-check-label" for="draw_ground">
								terrain
							</label>
						</div>
						<div class='col-10 mb-4'>
							<?php if(isset($terrains) && !empty(isset($terrains))): ?>
								...
								<div class="form-check form-check-inline">
									<input class="form-check-input mt-2" type="radio" name="draw_ground" id="<?= $composants[0] ?>" value='<?= $id ?>' <?php if(isset($old_input['draw_ground']) && $old_input['draw_ground'] == $id ):?> checked <?php endif ?>>
							...
							<?php else: ?>
								<p class='alert alert-warning'>
									...
									Aucun terrain à choisir.
								</p>
							<?php endif ?>
							<?php if(isset($errors['draw_ground'])):?>
							<div class='mt-2 alert alert-danger'>
								<ul class='mb-0'>
								<?php foreach($errors['draw_ground'] as $message): ?>
									<li><?= $message ?></li>
								<?php endforeach ?>
								</ul>
							</div>
						...
						<div class='col-2'>
							<label class="form-check-label mb-2 mr-3" for="draw_ground">
								Bâtiment
							</label>
						</div>
						<div class='col-10 mb-4'>
							<?php if(isset($batiments) && !empty(isset($batiments))): ?>
								<?php foreach($batiments as $id => $composants): ?>
								<div class="form-check form-check-inline">
									<input class="form-check-input mt-2" type="radio" name="draw_ground" id="<?= $composants[0] ?>" value='<?= $id ?>' <?php if(isset($old_input['draw_ground']) && $old_input['draw_ground'] == $id ):?> checked <?php endif ?>>
									<label class="form-check-label" for="<?= $composants[0] ?>">
										<span class='visually-hidden'><?= $composants[0] ?></span> <img src="../fond_carte/<?= $id  ?>.gif" width="34" height="34" alt='<?= $composants[0] ?>'>
						...
						<div class='col-10'>
							<?php if(isset($pnjs) && !empty(isset($pnjs))): ?>
								<?php foreach($pnjs as $id => $composants): ?>
								<div class="form-check form-check-inline">
									<input class="form-check-input mt-2" type="radio" name="draw_ground" id="<?= $composants[0] ?>" value='<?= $id ?>' <?php if(isset($old_input['draw_ground']) && $old_input['draw_ground'] == $id ):?> checked <?php endif ?>>
									<label class="form-check-label" for="<?= $composants[0] ?>">
										<span class='visually-hidden'><?= $composants[0] ?></span> <img src="../fond_carte/<?= $id  ?>.gif" width="34" height="34" alt='<?= $composants[0] ?>'>
		...
		<div class="col">
			<div class="card shadow">
				...
				<div class="card-body">
					<h5 class="card-title mb-3">Création d'une zone de terrain</h5>
				...
				<?php $i=0; for($y = $y_min; $y <= $y_max; $y++): ?>
					<tr>
						...
						<?php for($x = $x_min; $x <= $x_max; $x++):?>
							...
							<td class='tile ground-<?= strstr($map[$i]['fond_carte'],'.',true) ?><?php if($map[$i]['x_carte']==$x_choice OR $map[$i]['y_carte']==$y_choice):?> selected-line<?php endif ?><?php if($map[$i]['x_carte']==$x_choice):?> x<?php endif ?><?php if($map[$i]['y_carte']==$y_choice):?> y<?php endif ?>'>
