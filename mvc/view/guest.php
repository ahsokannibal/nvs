...
<div class="row justify-content-center">
	<div class="col-11 col-sm-3 mb-4 mb-sm-0">
		<div class='row pt-4'>
			...
			?>
			<form class='' action="?p=login" method="post" name="login" id="login">
				<?php if(isset($status)):?>
				<div class='mb-3'>
				...
				<div class="mb-3">
					...
					<input type="text" class="form-control form-control-sm <?php if(isset($errors['pseudo'])):?>is-invalid<?php endif ?>" id="pseudo" name='pseudo' placeholder="Pseudo" value='<?= $old_input['pseudo'] ?? ''?>'>
					<?php if(isset($errors['pseudo'])):?>
					<div class='mt-2 alert alert-danger'>
						<ul class='mb-0'>
						<?php foreach($errors['pseudo'] as $message): ?>
							<li><?= $message ?></li>
						<?php endforeach ?>
						</ul>
					</div>
				...
				<div class="mb-3">
					...
					<input type="password" class="form-control form-control-sm <?php if(isset($errors['password'])):?>is-invalid<?php endif ?>" id="password" name='password' placeholder="Mot de Passe">
				...
				<div class="mb-3">
					<label for="captcha_input" class='form-label'>Etes-vous un robot ?</label>
					<div class=''>
					...
					<div class='mt-2 alert alert-danger'>
						<ul class='mb-0'>
						<?php foreach($errors['captcha'] as $message): ?>
							<li><?= $message ?></li>
						<?php endforeach ?>
						</ul>
					</div>
	...
	<div class="col-12 col-sm-7">
		<div class='row justify-content-center'>
			<div class='col-11 col-sm shadow bg-gray-300 rounded-3 p-4 mx-2 mb-3 h-22'>
				...
				<p>
					<?= $nb_inscrits; ?> joueur(s) inscrit(s)<br />
					Dernier inscrit :<br/> <span class='<?= $dernier_inscrit->camp?> color fw-bold'><?= $dernier_inscrit->perso ?></span>
				</p>
				<p>
					Persos actifs : <br/><span class='nord color fw-bold'>nordistes : <?= $nord_actifs; ?></span> / <span class='sud color fw-bold'>sudistes : <?= $sud_actifs; ?></span>
				</p>
				<p>
					Vous voulez en découdre ?<br/>
					Engagez-vous soldat !<br/>
					<div class='mt-1'>
					<a href="?p=register" class="btn btn-light">S'inscrire</b></a>
					</div>
				</p>
			</div>
		...
		<div class='row justify-content-center'>
			<div class='col-11 col-sm shadow bg-gray-300 rounded-3 p-4 m-2'>
				...
				<p>
					...
					Au fur et à mesure de vos actions, vos capacités à commander se révéleront. Votre montée en grade vous permettra d'avoir encore plus de grouillots sous vos ordres.<br />
					Mais pour cela, il vous faudra utiliser tous les moyens disponibles : Relief du terrain, protection des bâtiments, achats d'armes et d'objets ainsi que le train à vapeur pour survivre au milieu du champ de bataille et des bêtes sauvages.<br /><br />
