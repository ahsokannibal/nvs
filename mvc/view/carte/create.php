...
<?php endif ?>
<?php if(isset($emptyCartes) && !empty($emptyCartes)): ?>
<div class="row row-cols-1 row-cols-md-2 g-4">
	<div class="col">
		<div class="card h-100 shadow">
			<div class="card-body">
				<h5 class="card-title mb-3">Créer une carte vierge</h5>
				<form class='row' name='virgin' method='post' action='?page=carte&action=store'>
					<div class='col-12 mb-3'>
						<select class="form-select <?php if(isset($errors['virgin_choix_carte'])):?>text-danger is-invalid<?php endif ?>" aria-label="Sélection de la carte" name="virgin_choix_carte">
							<OPTION value="0" disabled selected>Choix de la carte</option>
								<?php foreach($emptyCartes as $carte):?>
									<option value="<?= $carte ?>" <?php if(isset($old_input['virgin_choix_carte']) && $old_input['virgin_choix_carte'] == $carte):?> selected <?php endif ?>>carte <?= $carte ?></option>
								<?php endforeach ?>
						</select>
					...
					<div class='col-md-5'>
						<div class='row'>
							<div class='col-11 mb-3'>
								...
								<input class='form-control<?php if(isset($errors['virgin_creation_x_max'])):?> is-invalid<?php endif ?>' type='number' name='virgin_creation_x_max' id='virgin_creation_x_max' value='<?= $old_input['virgin_creation_x_max'] ?? ''?>'>
							...
							<div class='col-11'>
								...
								<input class='form-control <?php if(isset($errors['virgin_creation_y_max'])):?> is-invalid<?php endif ?>' type='number' name='virgin_creation_y_max' id='virgin_creation_y_max' value='<?= $old_input['virgin_creation_y_max'] ?? '' ?>'>
					...
					<div class='col-12 mt-3'>
						<input id="map" name="map" type="hidden" value="virgin" />
	...
	<div class="col">
		<div class="card h-100 shadow">
			<div class="card-body">
				...
				<form class='row' name='fromImg' method='post' action='?page=carte&action=store' enctype="multipart/form-data">
					<div class='col-12 mb-3'>
						<select class="form-select<?php if(isset($errors['fromImg_choix_carte'])):?> text-danger is-invalid<?php endif ?>" aria-label="Sélection de la carte" name="fromImg_choix_carte">
							<OPTION value="0" disabled selected>Choix de la carte</option>
								<?php foreach($emptyCartes as $carte):?>
									<option value="<?= $carte ?>" <?php if(isset($old_input['fromImg_choix_carte']) && $old_input['fromImg_choix_carte'] == $carte):?> selected <?php endif ?>>carte <?= $carte ?></option>
								<?php endforeach ?>
						</select>
					...
					<div class="mb-3">
						...
						<small class='text-muted'>Dimensions : 201x201</small>,
					...
					<div class='col-12'>
						<input type="hidden" id="map" name="map" value="fromImg" />
						<button type='submit' class='btn btn-primary'>Créer la carte</button>
					</div>
				</form>
			</div>
